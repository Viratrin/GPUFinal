/*
 * block-per-contract + pinned host memory (combines v2 and v3).
 *
 * Compile:
 *      nvcc -O2 --use_fast_math -o main_v4 main_v4.cu csv_parser.c -lm -lcurand
 *
 * Usage:
...
/*
 * One block = one contract.
 * All BLOCK_SIZE threads stride through N_PATHS and reduce via shared memory.
 */
...
{
    int contract = blockIdx.x;
    int tid = threadIdx.x;
    if (contract >= n_contracts) return;

    __shared__ float smem[BLOCK_SIZE];

    // load this contract's parameters into registers
    float s0 = d_S0[contract];
    float k = d_K[contract];
    float t = d_T[contract];
    float sigma = d_sigma[contract];
    ...
    curandState state;
    curand_init(1234ULL + (unsigned long long)(contract * BLOCK_SIZE + tid), 0ULL, 0ULL, &state);

    // each thread strides through a subset of paths and accumulates locally
    float local_sum = 0.0f;
    for (int i = tid; i < n_paths; i += blockDim.x) {
        float Z = curand_normal(&state); /* Z ~ N(0,1) */
        float ST = s0 * expf(drift + vol * Z); /* GBM terminal price */
        float payoff = ST - k;
        if (payoff > 0.0f) local_sum += payoff;
    }

    // reduce per-thread sums into a single block result via shared memory
    smem[tid] = local_sum;
    __syncthreads();
    ...
    if (tid == 0)
        d_prices[contract] = expf(-r * t) * (smem[0] / (float)n_paths);
}
...
int main(int argc, char *argv[]) {
    ...
    // parse CSV
    printf("\n=== Kaggle SPY Dataset Pricing (v4: block-per-contract + pinned memory) ===\n");
    ...
    // extract the data from the struct-of-arrays (pinned so DMA can skip staging)
    float *h_S0, *h_K, *h_T, *h_sigma, *h_prices;
    CUDA_CHECK(cudaMallocHost(&h_S0, n * sizeof(float)));
    CUDA_CHECK(cudaMallocHost(&h_K, n * sizeof(float)));
    CUDA_CHECK(cudaMallocHost(&h_T, n * sizeof(float)));
    CUDA_CHECK(cudaMallocHost(&h_sigma, n * sizeof(float)));
    CUDA_CHECK(cudaMallocHost(&h_prices, n * sizeof(float)));
    ...
    // launch kernel
    printf("Launching kernel: %d blocks x %d threads (~%d paths/thread)\n",
           n, BLOCK_SIZE, N_PATHS / BLOCK_SIZE);
    ...
    // cleanup (pinned memory must be freed with cudaFreeHost)
    cudaFree(d_S0);
    cudaFree(d_K);
    cudaFree(d_T);
    cudaFree(d_sigma);
...
// Mean absolute error    : $2.6834
// Prices inside spread   : 59308 / 500000  (11.9%)
// Throughput             : 368935.39 contracts/sec
